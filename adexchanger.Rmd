...
library(topicmodels)
library(knitr)
#setwd('/Users/markdavenport/Documents/adexchanger/')
#dtm <- load('/Users/markdavenport/Documents/adexchanger/dtm.RData')
load('dtm.RData')
terms <- findFreqTerms(dtm, 2000)
```

Most frequenct terms:
```{r echo=FALSE }
terms
...
#               options = NULL)
selectizeInput('termToExplore', label = NULL, choices = colnames(dtm),
...
fluidPage(
    ...
    fluidRow(
      column(4, 
          selectizeInput(
        'termToExplore2', 'Second term', choices = colnames(dtm)
        )
      ),
      column(4, 
          selectInput(
        'termToExplore1', 'First term', choices = colnames(dtm)
        )
      )     
    )
)
